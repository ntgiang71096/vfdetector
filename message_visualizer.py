...
def visualize():
    ...
    url_to_mes_test_prob = variant_ensemble.read_prob_from_file('probs/message_prob_test.txt')
    ...
    for id, url in id_to_test_url.items():
        if id in neg_ids:
            neg_test.append(message_test[id])

        y_pred = 1 if url_to_mes_test_prob[url] >= 0.5 else 0
    ...
    with open('message_explanation.json', 'r') as file:
    ...
    plt.savefig('imgs/message_vuln_in_non_vuln_commit.png')
    plt.close()
    ...
    plt.xlabel("Occurrence")
    plt.savefig('imgs/message_tp_pos_count.png')
    plt.close()
    ...
    plt.xlabel("Occurrence")
    plt.savefig('imgs/message_tp_neg_count.png')
    plt.close()
    ...
    plt.xlabel("Occurrence")
    plt.savefig('imgs/message_tn_pos_count.png')
    plt.close()
    ...
    plt.xlabel("Occurrence")
    plt.savefig('imgs/message_tn_neg_count.png')
    plt.close()
    ...
    plt.title('Fig 1. Highest weight\'s term distribution on message classifier')
    plt.ylabel('Highest weight')
    plt.xlabel('Segment')
    plt.savefig('imgs/message_weight_plot.png')
    plt.close()
